...
class NetProducts:
    def __init__(self, vehicleCapacity, folder):
        ...
        df_offices = pd.read_csv(
            folder + 'offices.csv',
            usecols=['office_id', 'transfer_price', 'transfer_max'],
            dtype={'office_id': int, 'transfer_price': float, 'transfer_max': float}
        )
        df_reqs = pd.read_csv(
            folder + 'reqs.csv',
            dtype={'src': int, 'dst': int, 'volume': float}
        )
        df_distance_matrix = pd.read_csv(
            folder + 'distance_matrix.csv',
            usecols=['src', 'dst', 'price'],
            dtype={'src': int, 'dst': int, 'price': float}
        )

        for i, row in df_offices.iterrows():
            self.storageId.append(int(row['office_id']))
            self.storages[int(row['office_id'])] = {
                "overloadPrice": row['transfer_price'],
                "maxOverload": row['transfer_max']
            }
