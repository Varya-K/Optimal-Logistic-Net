...
from Solver import SolverResult, SolveMILP
...
class VNS:
    ...
    def InitSolution(self):
        self.data = {
            productId: [Path(path=[info["source"], info["destination"]])]
            for productId, info in self.net.products.items()
        }
        self.result = SolveMILP(self.net, self.data, 300)
    ...
    def ProcessNewPaths(self, newPaths, maxTime):
        ...
        if success:
            ...
            newResult = SolveMILP(self.net, newData, maxTime, initSolution=self.result)
    ...
    def ProccessAddNewShortestPathsNeighborhood(self, newPathsCount, random, maxTime):
        newPaths = {
            productId: self.shortestPaths.GetNewShortestPaths(productId, random, newPathsCount, self.data)
            for productId in self.net.productId
        }
    ...
    def ProccessAddNewShortestPathsAvoidingStoragesNeighborhood(self, newPathsCount, minCount, overloadTreshold, maxStoragesCount, maxTime):
        ...
        overload = [
            (sumOverload[storageId] / info["maxOverload"], info["overloadPrice"], storageId)
            for storageId, info in self.net.storages.items()
        ]
        ...
        for i in range(startI, endI):
            ...
            for productId, info in self.net.products.items():
                newPaths[productId] += self.shortestPaths.GetShortestPathsFromTempGraph(
                    productId, info["source"], info["destination"], self.data,
                    anotherData=newPaths, count=newPathsCount
                )
    ...
    def ProccessAddNewShortestPathsThroughUsedEdgesNeighborhood(self, newPathsCount, vehicleCoeff, maxTime):
        ...
        edgeFlow = {
            edge: flow % self.net.vehicleCapacity if Math.Bg(flow % self.net.vehicleCapacity, 0) else self.net.vehicleCapacity
            for edge, flow in edgeFlow.items()
        }

        newEdgesPrices = {
            edge: self.net.storages[edge[0]]["overloadPrice"]
                  + self.net.storages[edge[1]]["overloadPrice"]
                  + (flow / self.net.vehicleCapacity * (self.net.edgesPrices[edge] / self.net.vehicleCapacity) ** 0.5
                     if Math.Sm(flow, self.net.vehicleCapacity)
                     else self.net.edgesPrices[edge] / self.net.vehicleCapacity)
            for edge, flow in edgeFlow.items()
        }
        ...
        for productId, info in self.net.products.items():
            ...
            sumFlow = 0
            newPaths[productId] = self.shortestPaths.GetShortestPathsFromTempGraph(
                productId, info["source"], info["destination"], self.data, needNext=needNext
            )
